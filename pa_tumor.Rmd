---
title: "Analysis of PA populations"
...
```{r}
so <- qread(file.path("objects", "so_tumor_pa2.qs"), nthreads = 4)
so_old <- qread(file.path("objects", "so_tumor_pa.qs"), nthreads = 4)
```
...
The clustering is mildy-patient specific, which contrasts with the GG samples, which form largely patient specific clusters. 
...
```{r}
plot_umap(so, "pa_tumor_res.0.1", legend_title = "clusters") 
```
...
```{r}
plot_harmony(so, "pa_tumor_harmony_res.0.3", legend_title = "harmony-clusters") 
```
...
# Identifies upregulated marker genes for each cluster
markers.cell_type.up <- findMarkers(sce, 
                                groups = sce$pa_tumor_harmony_res.0.3, 
                                block = sce$orig.ident, # Accounts for batch effects using the orig.ident column
                                lfc = 0.5,
                                direction="up") %>% 
...
topx <- mkrs$mkrs %>% 
  ...
  mutate(group = factor(group, levels = sort(unique(so$pa_tumor_harmony_res.0.3)))) %>% 
  group_by(group) %>% 
...
hm <- plot_heatmap(ScaleData(so, features =  topx_features),
             ...
             group = "pa_tumor_harmony_res.0.3",
             average = TRUE,
...
library(canatlas) # Cannot install
...
res <- clustify(so,
                ref_mat = atlas,
                cluster_col = "pa_tumor_harmony_res.0.3", 
                obj_out = FALSE)
...
mkrs <- read_tsv(file.path("markers", 
                           "pa_harmony_cluster_markers.tsv")) %>% 
  filter(logFC > 0,
         pct_in > 10,
         padj < 0.05) %>% 
  group_by(group) %>% 
...
go_res[["result"]] %>% 
  ...
  write_tsv(., file.path(tbls_dir, "goterms_pa_harmony_clusters.tsv"))
go_res[["result"]] %>% 
  mutate(parents = unlist(map(parents, ~str_c(.x, collapse = ","))),
  ...
  split(., .$query) %>% 
  openxlsx::write.xlsx(., 
                       file.path(tbls_dir, "goterms_pa_harmony_clusters.xlsx"),
                       overwrite = TRUE)
```
...
```{r, rows.print = 50}
cell_count_mat <- get_cell_count_matrix(so, "UPN", "pa_tumor_harmony_res.0.3")  %>% 
  rownames_to_column("tumor") 

write_csv(cell_count_mat, file.path(tbls_dir, "pa_tumor_harmony_cell_counts.csv"))
cell_count_mat
```
...
plot_cell_proportions(so, 
                      sample_id = "UPN",
                      group_id = "pa_tumor_harmony_res.0.3"
                      ) +
  labs(x = "UPN",
       fill = "")
```
...
```{r}
plot_cell_proportions(so, 
                      sample_id = "pa_tumor_harmony_res.0.3",
                      group_id = "UPN"
                      ) +
  labs(x = "cluster",
       fill = "")
...
## Marker gene lists Huang et al

A single cell RNA-seq dataset was generated from cortical OPC during human brain development (https://www.cell.com/cell/fulltext/S0092-8674(20)30759-5 ) . They characterized their populations using the following markers. 

Note that the Pre-OPC progenitor is marked by NPC related genes and is produced from outer radial glial cell populations. The Pre-OPC progenitor might be analgous to the Bergman Glia in Cerebellum? 

```{r}
knitr::include_graphics(file.path(doc_dir, "huang_et_al", "fx1.jpg"))
```
...
Markers of Lake et al (https://www.nature.com/articles/nbt.4038#Sec35 )
...
```{r}
plot_harmony(so, "pa_tumor_harmony_res.0.3")
```
...
Examining similarity to relevant published cells types shows that the clusters (excluding 3 and 7) are mostly similar to other tumor populations or oligodendrocytes. 
  
cluster 0: More differentiated OPCs (express MOG and PLP1)  
cluster 1: Intermediate OPC population, which MAPK signaling, NPC-like signatures and differentiated OPC signatures.   
cluster 2: Protocadherin gamma expressing subpopulation  
cluster 3: Patient specific cluster from UPN 1459   
cluster 4: High MAPK signaling  
cluster 5: NPC-like expression, could be more of a progenitor to OPCs  
cluster 6: VEGF+ subpopulation, likely representing a hypoxic population, (see below)    
cluster 7: Not sure  
cluster 8: Contaminating myeloid cells that will be excluded.  

labels v1
```{r}
old_labels <- so_old$pa_subpopulation[intersect(colnames(so), colnames(so_old))]
...
```{r}
# pa_subpopulation <- c(
#   "0" = "Differentiatied (MOG+ PLP1+)",
#   "1" = "Intermediate (ASCL1+ JUNB+ SOX3+)",
#   "2" = "PCDHG+",
#   "3" = "1459-specific (SIX1/6+)",
#   "4" = "MAPKhi",
#   "5" = "NPC-like (VIM+ HOPX+ GFAP+)",
#   "6" = "Hypoxic (VEGFA+ ADM+ ENO1+)",
#   "7" = "TNC+ FOXD3+",
#   "8" = "Myeloid"
# )
...
so$pa_subpopulation <- pa_subpopulation[as.character(so$pa_tumor_harmony_res.0.3)]

plot_harmony(so, "pa_subpopulation")
```
```{r}
so <- subset(so, subset = pa_subpopulation %in% c("Myeloid"), invert = T)
```
...
go_res[["result"]] %>% 
  ...
  split(., .$query) %>% 
  openxlsx::write.xlsx(., 
                       file.path(tbls_dir, "goterms_pa_harmony_subpopulations.xlsx"),
                       overwrite = TRUE)
```
...
```{r, rows.print = 50}
cell_count_mat <- get_cell_count_matrix(so, "UPN", "pa_subpopulation")  %>% 
  rownames_to_column("tumor") 

write_csv(cell_count_mat, file.path(tbls_dir, "pa_tumor_harmony_subpopulation_cell_counts.csv"))
cell_count_mat
```

```{r, fig.width=7}
p <- plot_harmony(so, c("GFAP", "VIM", "HOPX", "ID3")) %>% 
  plot_grid(plotlist = ., nrow = 2, ncol = 2)
p
save_plot(file.path(fig_dir, "umap_nes_pa.pdf"), p, nrow = 2, ncol = 2, base_asp = 1)
```


```{r}
p <- plot_cell_proportions(so, 
                      sample_id = "pa_subpopulation",
                      group_id = "UPN"
                      ) +
  labs(x = "cluster",
       fill = "")
...
save_plot(file.path(fig_dir, "pa_subpop_cell_proportions.pdf"),
          p,base_height = 6,
          base_asp = 1.1)
```
...
markers.cell_type.up %>% 
  bind_rows(.id = "cluster") %>% 
  write_tsv(file.path(mkrs_dir, "scran_pa_tumor_subpopulation_consensus_markers.tsv"))
...
map(markers.cell_type.up, 
    ~set_xlsx_class(.x, "gene", "Text")) %>% 
openxlsx::write.xlsx(., file.path(mkrs_dir, "scran_pa_tumor_subpopulation_consensus_markers.xlsx"),
                     overwrite = TRUE)
...
```{r}
library(clustree)
p <- clustree(so, prefix = "pa_tumor_harmony_res.") +
  scale_color_manual(values = discrete_palette_default)
p
save_plot(file.path(fig_dir, "pa_harmony_clustree_0.3_selected.pdf"), 
          p,
          base_asp = 1.5,
          base_height = 8)
```
...
```{r, eval = TRUE}
qsave(so, file.path("objects", "so_tumor_pa2.qs"))

#so <- qread(file.path("objects", "so_tumor_pa2.qs"))
```
